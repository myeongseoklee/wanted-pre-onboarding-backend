...
- recruitment-notice-show.dto.ts: 채용공고 상세 데이터 dto와 해당 공고를 올린 회사가 채용중인 공고 목록의 데이터 dto를 담는 dto입니다 - recruitment-notice.dto.ts: 채용공고 엔티티의 데이터 중 view에 필요한 데이터를 담고 가공하는 dto입니다. 근무지 정보(city,province)를 결합하여 workingArea 만들고, view에서는 불필요한 정보들을 제외하였습니다.
- 응답용 dto 필드의 값을 불변으로 주기 위해, 필드에 private readonly 접근 제한자를 주고, getter를 정의하였습니다.

[service]

- 채용공고 상세내용 조회: 채용 회사의 id로 채용공고 엔티티의 모든 필드를 조회합니다. 이번에는 find operator 메소드를 사용하였습니다. 채용공고 엔티티와 릴레이션이 설정된 직무(job), 회사(company), 근무지(city,province) 엔티티도 join하여 조회하였고, 쓰이지 않을 것으로 예상되는 데이터는 select에서 제외하였습니다.

- 회사의 다른 채용공고 목록 조회 : 회사가 올린 다른 채용 공고를 추가적으로 포함하는 방법은 채용상세 정보 조회 쿼리에 포함하여 한 번에 가져오는 방법과, 상세 정보를 가져온 후, 별도로 채용공고 목록을 가져와 서비스단에서 데이터를 붙이는 방법이 있습니다. 전자의 방법은 RDBMS의 JOIN과 SUBQUERY를 사용하면 됩니다. 하지만, 기존의 채용 목록 조회 api에서 사용한 코드를 재활용할 수 있고, 회사가 올린 다른 채용공고를 보여주는 요구사항이 '이 공고와 유사한 다른 회사의 채용 공고'를 보여주는 요구사항으로 변화 했을때 대응이 수월할 것 같다는 점. 그리고 채용 목록 조회 api에서 제공하는 데이터로도 충분할 것으로 예상되므로 후자의 방식으로 구현했습니다.

[repository]

- 레포지토리 메소드도 서비스 메소드처럼 채용공고 목록 조회에서 사용한 코드를 함수(getPaginationQuery)로 분리하여 재사용하였습니다.
